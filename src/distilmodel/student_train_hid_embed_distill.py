...
def train_with_hid_embed_distill(student_model, iterator, optimizer, scheduler, criterion, teacher_model,
    ...
    for i, batch in enumerate(iterator):
        ...
        # hidden states
        for student_layer_id, teacher_layer_id in layer_config["student_to_teacher_layer"].items():
            non_trainable_layernorm = nn.LayerNorm(teacher_outputs["hidden_states"][teacher_layer_id + 1].shape[1:],
                                                   elementwise_affine=False)
            teacher_hidden, student_hidden = teacher_outputs["hidden_states"][teacher_layer_id + 1], \
                student_outputs["hidden_states"][int(student_layer_id) + 1]
        ...
        full_loss = (hidden_loss + embedded_loss) / (float(len(layer_config["student_to_teacher_layer"])) + 1)
...
if __name__ == '__main__':
    ...
    for epoch in trange(args.epochs, desc="Epoch"):
        train_loss, distill_loss, train_acc, wall_time = train_with_hid_embed_distill(bert_student_model,
                                                                                      train_dataloader,
                                                                                      optimizer, scheduler,
    ...
    if args.logdir is not None:
        ...
        config_new_path, result_new_path = get_logs_path(args.logdir, time_now, exp_type, comfig_name,
                                                         args.layer_config)
        # config_new_path = os.path.join(args.logdir, time_now + "_student_" + exp_type + "_" + comfig_name +
        #                                '_config.json')
        # result_new_path = os.path.join(args.logdir, time_now + "_student_" + exp_type + "_" + comfig_name +
        #                                '_results.json')
        # config_new_path = os.path.join(args.logdir, conf_path)
        # result_new_path = os.path.join(args.logdir, res_path)

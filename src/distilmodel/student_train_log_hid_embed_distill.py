...
pth_sep = os.path.sep
exp_type = "log_hid_embed"


def train_with_log_hid_emb_distill(student_model, iterator, optimizer, scheduler, criterion, teacher_model,
    ...
    for i, batch in enumerate(iterator):
        ...
        logit_loss += cross_entropy_loss.item()
        ...
        full_loss = (logit_loss + hidden_loss + embedded_loss) / (
                float(len(layer_config["student_to_teacher_layer"])) + 2)
...
if __name__ == '__main__':
    ...
    for epoch in trange(args.epochs, desc="Epoch"):
        train_loss, distill_loss, train_acc, wall_time = train_with_log_hid_emb_distill(bert_student_model,
